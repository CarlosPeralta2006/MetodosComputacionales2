...
axes[0].plot(Mo[kV1]['Energía'], Mo[kV1]['Fotones'], 'b-', linewidth=1.5)
axes[0].set_title(f'Molibdeno (Mo) a {kV1}', fontsize=14)
axes[0].set_xlabel('Energía (keV)', fontsize=12)
axes[0].set_ylabel('Conteo de fotones (u.a.)', fontsize=12)
axes[0].grid(True, linestyle='--', alpha=0.6)
axes[0].set_xlim(0, 35)
axes[0].text(0.5, 0.95, f'Datos: {len(Mo[kV1])} puntos', 
            transform=axes[0].transAxes, ha='center', 
            bbox=dict(facecolor='white', alpha=0.8))

# ---- Gráfico 2: Rodio (Rh) ----
axes[1].plot(Rh[kV2]['Energía'], Rh[kV2]['Fotones'], 'r-', linewidth=1.5)
axes[1].set_title(f'Rodio (Rh) a {kV2}', fontsize=14)
axes[1].set_xlabel('Energía (keV)', fontsize=12)
axes[1].grid(True, linestyle='--', alpha=0.6)
axes[1].set_xlim(0, 35)
axes[1].text(0.5, 0.95, f'Datos: {len(Rh[kV2])} puntos', 
            transform=axes[1].transAxes, ha='center', 
            bbox=dict(facecolor='white', alpha=0.8))

# ---- Gráfico 3: Tungsteno (W) ----
axes[2].plot(W[kV3]['Energía'], W[kV3]['Fotones'], 'g-', linewidth=1.5)
axes[2].set_title(f'Tungsteno (W) a {kV3}', fontsize=14)
axes[2].set_xlabel('Energía (keV)', fontsize=12)
axes[2].grid(True, linestyle='--', alpha=0.6)
axes[2].set_xlim(0, 35)
axes[2].text(0.5, 0.95, f'Datos: {len(W[kV3])} puntos', 
            transform=axes[2].transAxes, ha='center', 
            bbox=dict(facecolor='white', alpha=0.8))

# Guardar y mostrar
plt.tight_layout()
plt.savefig("222.pdf", bbox_inches='tight')
plt.show()

# Mostrar información de los DataFrames
for elemento, df in zip(['Mo', 'Rh', 'W'], [Mo[kV2], Rh[kV3], W[kV3]]):
